...
@dataclass
class FixedCorpusCfg(CorpusCfg):
    """
    固定文本语料库配置
...
class FixedCorpus(Corpus):
    """
    总是返回固定文本的语料库
    """

    def __init__(self, cfg: "FixedCorpusCfg"):
        super().__init__(cfg)
        self.cfg: FixedCorpusCfg
...
def fixed_text_data(text: str, selected_effects: list, perspective_x: float, perspective_y: float, perspective_z: float):
    effects = []
    for effect in selected_effects:
        if effect == "Line":
            effects.append(Line(0.9, thickness=(2, 5)))
        elif effect == "DropoutRand":
            effects.append(DropoutRand())
        elif effect == "DropoutVertical":
            effects.append(DropoutVertical())
        elif effect == "Padding":
            effects.append(Padding())
        elif effect == "DropoutHorizontal":
            effects.append(DropoutHorizontal())
        elif effect == "Emboss":
            effects.append(Emboss(alpha=(0.0, 1.0), strength=(0.5, 1.5)))
        elif effect == "MotionBlur":
            effects.append(MotionBlur())
        elif effect == "Curve":
            effects.append(Curve(p=1))
    ...
    return GeneratorCfg(
        ...
        render_cfg=RenderCfg(
            ...
            height=32,
            perspective_transform=NormPerspectiveTransformCfg(perspective_x, perspective_y, perspective_z),
            corpus=FixedCorpus(
                FixedCorpusCfg(
                    text=text,
...
def get_configs(text: str, selected_effects: list, perspective_x: float, perspective_y: float, perspective_z: float):
    return [fixed_text_data(text, selected_effects, perspective_x, perspective_y, perspective_z)]